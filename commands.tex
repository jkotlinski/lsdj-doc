...
Slides to the target note in the given duration. The actual time depends on the instrument's \textsc{p+l+vib} setting; in \textsc{hf} mode the duration is n/360 seconds, otherwise the duration is in ticks. If the duration is set to 0, the slide will happen instantly without stopping the P command.
...
The actual speed depends on the instrument's \textsc{p+l+vib} setting. In \textsc{hf} mode, the pitch will be updated at 360 Hz, otherwise it changes every tick.
