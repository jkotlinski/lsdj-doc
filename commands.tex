...
The L command works on pulse and wave instruments. It slides to the note with a given duration (0 is fastest, \$FF is slowest). The duration differs depending on whether the instrument is set to normal or HF vibrato.
...
The pitch command works on pulse and wave channels. When used on \texttt{KIT} instruments, it sounds more like a pitch shifter. The actual speed depends on whether the instrument is set to HF or normal vibrato.
...
The Z command repeats the last non-Z command, adding a random number to the original command value. The Z command value is a bit mask, meaning that ZF0 would add some value 00, 10, 20, 30, ..., to the original value.
