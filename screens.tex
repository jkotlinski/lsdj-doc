...
\begin{description}
    ...
    \item[\textsc{p/l/v}] Controls the behavior of \textsc{p}, \textsc{l} and \textsc{v} commands. \textsc{A+u/d} switches pitch update speed: \textsc{fast} updates pitch at 360 Hz, \textsc{tick} updates pitch every tick, \textsc{step} is like \textsc{fast} except that \textsc{p} will do a pitch change instead of a pitch bend. \textsc{A+l/r} changes vibrato shape between downwards triangle, saw and square, and upwards triangle, saw and square.
    \item[\textsc{tuning}]
        \begin{description}
        ...
        \end{description}
    \item[\textsc{table}] Selects a table to run when a note is played. If you want to edit the table, press \textsc{select+right} to go to the table screen. To create a new table, press \textsc{a,a}. To clone the table, press \textsc{select+(b,a)}.
...
\begin{description}
	...
	\item[\textsc{s cmd}] When set to \textsc{free}, noise changing commands can randomly\footnotemark mute the sound. When set to \textsc{stable}, commands are limited so that sound will never be muted by accident.
...
slave in sync mode, it is possible to temporarily play a little faster or slower by pressing \textsc{a+left/right}.
    ...
    \item[\textsc{drm max}] Adjusts the range of \textsc{drm} tuning (section~\ref{general-instrument-parameters}). By default, note B-A in \textsc{drm} tuning equals B-A in \textsc{12t} tuning. Use this setting if you want to map \textsc{drm} B-A to a lower note.
