...
\subsection{Useful keypresses}

\begin{description}
	\item[\textsc{A}] Insert chain on empty step.
	\item[\textsc{A,a}] Add new chain.
	\item[\textsc{Select+right}] Edit chain.
	\item[\textsc{B+a}] Remove chain. On empty step, pull up down-below chains.
	\item[\textsc{Start}] Start/stop playing.
	\item[\textsc{Select+start}] Instant restart.
	\item[\textsc{Left+start}] Queue row for playback.
	\item[\textsc{B,b,b}] Mark row.
	\item[\textsc{B+up/down}] Jump to nearest marked row, or page up/down.
\end{description}
...
While editing a chain, \textsc{B+up/down} jumps to the next or previous chain, and \textsc{B+left/right} jumps to the chain of a neighboring channel.
...
%\marginpar{\includegraphics[width=0.8cm]{tip}TIP!}
\end{itemize}
