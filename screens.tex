...
The command column is used to add effects. For example, the K command kills the sound of the channel. Tap \textsc{A,A} on a command for quick-help on what the command does.
...
These parameters are used in most instrument types. Tap \textsc{A,A} on any parameter for quick-help on what the parameter does.
...
\begin{description}
	...
	\item[\textsc{shape}] Noise generator control. The first digit changes pitch by octave, the second digit divides the frequency. Set the second digit to 0-7 for periodic noise, 8-\$F for random noise.
        An in-depth technical explanation of the noise generator can be found in \href{https://www.devrs.com/gb/files/hosted/GBSOUND.txt}{gbsound.txt}.
...
\includegraphics[width=0.8cm]{tip}TIP!
\begin{itemize}
        \item \textit{There is a useful file manager application available at} \url{https://littlesounddj.com/lsd/latest/lsd-manager/}.
	\end{itemize}
\end{figure}
